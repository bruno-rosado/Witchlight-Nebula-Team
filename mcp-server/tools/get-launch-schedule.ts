  ...
  server.registerTool(
    ...
    async ({ limit = 10, location, agency, status }) => {
      try {
        ...
        if (location) params.append('location__name__icontains', location);
        if (agency) params.append('rocket__configuration__manufacturer__name__icontains', agency);
        if (status && status !== 'all') params.append('status__abbrev', status);
        ...
        const launchDescriptions = launches.map((launch: any, index: number) => {
