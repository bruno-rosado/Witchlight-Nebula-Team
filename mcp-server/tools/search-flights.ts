  ...
  server.registerTool(
    ...
    async ({ departure, arrival, outbound_date, return_date, adults = 1, travel_class = "economy" }) => {
      try {
        ...
        const flights = (data.best_flights || []).concat(data.other_flights || []).map((item: any) => {
          ...
          return {
            ...
            layovers: item.layovers?.map((l: any) => `${l.name} (${l.duration} min)`),
        ...
        const flightDescriptions = flights.map((flight: any, index: number) => {
        ...
        const classText = travel_class.replace('_', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase());
