...
export function register(server) {
    server.registerTool("correlate-mission-data", {
        ...
        try {
            ...
            if (latitude !== undefined && longitude !== undefined) {
                nearbyEvents = (eonetData.events || []).filter((event) => {
            ...
            const upcomingLaunches = (launchData.results || []).slice(0, 5).map((launch) => ({
                name: launch.name,
                date: launch.net,
            ...
            if (nearbyEvents.length > 0) {
                const severeEvents = nearbyEvents.filter((e) => ['Wildfires', 'Severe Storms', 'Volcanoes'].includes(e.category));
            ...
            const recentSpaceWeather = spaceWeatherEvents.filter((e) => {
                const eventDate = new Date(e.date);
                const now = new Date();
                const daysDiff = (eventDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);
                return daysDiff >= -2 && daysDiff <= daysAhead;
            });
            const severeFlares = recentSpaceWeather.filter((e) => e.type === 'Solar Flare' && (e.severity?.startsWith('M') || e.severity?.startsWith('X')));
            if (severeFlares.length > 0) {
                risks.push(`☀️ ${severeFlares.length} M/X-class solar flare(s) detected`);
                if (riskLevel === 'LOW')
                    riskLevel = 'MEDIUM';
            }
            const severeStorms = recentSpaceWeather.filter((e) => e.type === 'Geomagnetic Storm' && e.severity >= 5);
            ...
            if (nearbyEvents.length > 0) {
                ...
                nearbyEvents.slice(0, 5).forEach((event, i) => {
            ...
            if (recentSpaceWeather.length > 0) {
                ...
                recentSpaceWeather.slice(0, 5).forEach((event, i) => {
