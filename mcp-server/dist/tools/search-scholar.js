...
export function register(server) {
    server.registerTool("search-scholar", {
        ...
        try {
            ...
            if (year_low)
                params.append('as_ylo', String(year_low));
            if (year_high)
                params.append('as_yhi', String(year_high));
            ...
            const results = (data.organic_results || []).map((item) => ({
                ...
                authors: item.publication_info?.authors?.map((a) => a.name).join(', '),
                ...
                pdf_link: item.resources?.find((r) => r.file_format === 'PDF')?.link
            }));
            if (results.length === 0) {
                return {
            ...
            const resultDescriptions = results.map((result, index) => {
