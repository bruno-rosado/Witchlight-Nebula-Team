    ...
    server.registerTool("fetch-weather", {
        ...
        try {
            ...
            const tempCandidate = findFirst(data, (k, v) => {
                ...
                return ((key.includes('temp') || key.includes('temperature')) && (typeof v === 'number' || !Number.isNaN(Number(v))));
            });
            let temperature = 0;
            if (typeof tempCandidate === 'number')
                temperature = tempCandidate;
            else if (tempCandidate !== undefined)
                temperature = Number(tempCandidate) || 0;
            // Heuristics for conditions
            const condCandidate = findFirst(data, (k, v) => {
                const key = String(k).toLowerCase();
                return ((key.includes('condition') || key.includes('conditions') || key.includes('description') || key.includes('weather') || key.includes('text'))
                    && (typeof v === 'string'));
            });
